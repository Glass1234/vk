...
export class Api {
    ...
    async #getRequest(apiMethod, data = null) {
        const url = `${this.#baseUrl + apiMethod}?access_token=${this.#token}&v=5.131${data === null ? '' : data}`
        return (await axios({
            method: 'get',
            url: url
        }))
    ...
    async getUserPeople(id) {
        const data = `&order=hints&user_id=${id}`
